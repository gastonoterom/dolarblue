...
def pub_update_values() -> None:
    """Publishes a 'update-dolarblue-values' with an 'update-request' message to the pubsub manager."""
    publish_function("update-dolarblue-values", "update-request")


def pub_values_updated(sources_updated: dict) -> None:
    """Publishes a 'dolarblue-values-updated' message to the pubsub manager, with the sources updated information
    ...
    try:
        ...
        raise TypeError("sources_update dictionary is NOT json serializable") from error
