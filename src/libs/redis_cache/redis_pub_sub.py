...
class RedisSub:
    ...
    def __init__(
        ...
        """Needs a redis lower level pubsub object, if none is provided one is obtained from the projects pool factory"""
    ...
    def subscribe(self, chann: str, handler: Callable):
        """Subscribes the redis pubsub object to a specified channel with a handler function, requires an open
        pubsub connection."""
    ...
    def unsubscribe(self, chann: str):
        """Unsubscribe all the subscribers for a specific channel provided. Also stops all their respective threads.

        The object must have previously been subscribed or it will raise a SubscriptionNotFoundException."""

        if self.threads.get(chann) is None:
            raise SubscriptionNotFoundException("Can't unsubscribe from %s as it was never subscribed to", chann)
...
class RedisPub:
    ...
    def __init__(
        ...
        """Needs a redis connection, if none is provided one is obtained from the projects factory"""
